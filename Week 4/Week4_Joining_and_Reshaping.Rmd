...
forest_long <- forest %>%
  pivot_longer(
    cols = -country, #select all columns except country; all years
...
# BUT! the years are stored as *text* with the 'x' character before the number.
...
ggplot(country_summary, aes(x = mean_temp, y = mean_forest, color = continent)) +
  geom_point() +
  labs(title = "Average Forest Cover vs. Mean Annual Temperature (by Country)",
