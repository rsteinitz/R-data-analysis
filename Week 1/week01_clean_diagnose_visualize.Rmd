...
options(width = 300) # Or a sufficiently large number
```
...
In this course, we will use the `penguins` dataset from the `palmerpenguins` package. These data contain physical measurements, species info, and other metadata on three penguin species in Antarctica.  

We will use the RAW penguin data, which includes errors, miscategorizations, and missing data. The idea is that is mimics the type of data you might work with in your research.  
...
# what type of data exists in a particular column
class(data$island) # character
class(data$date_egg) # character
...
unique(data$date_egg) # 50 unique dates... 
...
# this is also an option per specific column
...
Instead, I prefer to use ggplots ('GG' stands for Grammar of Graphics).  
...
- **Aesthetics** (aes) – mappings that connect variables to visual properties (like x, y, color)
- **Geometries** (geoms) – the type of plot you want to make (like geom_point() or geom_bar()).
...
### 5.2 Bar Plot of a Categorical Variable
...
Next week, we’ll start wrangling our data using `filter()`, `select()`,
and `mutate()` from `dplyr`.  
